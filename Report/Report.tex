...
\textbf{Mode 1—Blind (Estimated):} Pure odometry spiraled catastrophically. \textit{Critical:} When heading error exceeded $\pi/2$ (90°), control became unstable—the internal map inverted, causing the agent to walk \textit{away} while believing it approached home.

\textbf{Mode 2—Control (True):} Ground truth confirmed perfect homing with error-free odometry.

\textbf{Mode 3—Sun Compass:} Periodic scans created ``sawtooth'' errors—drift reset before reaching $\pi/2$. Error reduced by 90\%, achieving 100\% success rate.
...
\begin{figure}[H]
    ...
    \caption{Simulation interface showing parameter controls (left), 3D trajectory (top center), and true vs. estimated position with error metrics (bottom).}
    \label{fig:ui}
\end{figure}
...
I designed a \textbf{modular, decoupled architecture} where the physics engine, mathematical models, and agent logic operate independently. This strict separation ensures high customizability and establishes an \textbf{RL-ready environment}; the exposed state-action interfaces allow seamless integration with reinforcement learning algorithms to train optimized navigation policies, directly supporting future research into autonomous warehouse logistics.
